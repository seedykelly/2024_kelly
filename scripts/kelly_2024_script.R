...
mobility_rep <- bf(distance.z ~ sex.centred + observation.n + sex.centred*observation.n + (observation.n|ID), family = gaussian)

fit.model.brms.mobility <- brm(mobility_rep, data = morphology_long, save_pars = save_pars(all = TRUE), 
                             warmup=100, iter=2000, seed=12345, thin=2, chains=4, cores= 4)
summary(fit.model.brms.mobility,prob = 0.95)
